...
do -- OPie:RadioSet
	...
	function RadioSet:SetOptionText(index, text)
		...
		assert(type(index) == "number" and index % 1 == 0 and index >= 1 and type(text) == "string",
			'Syntax: RadioSet:SetOptionText(index, "text")')
	...
	function RadioSet:Reflow(targetWidth)
		...
		assert(targetWidth == nil or type(targetWidth) == "number" and targetWidth > 0,
			'Syntax: RadioSet:Reflow([targetWidth])')
...
do -- OPie:OptionsSlider
	...
	function OptionsSlider:SetRangeLabelText(lo, hi)
		assert((lo == nil or type(lo) == "string") and (hi == nil or type(hi) == "string"),
			'Syntax: OptionsSlider:SetRangeLabelText("lo", "hi")')
		local d = assert(getWidgetData(self, OptionsSliderData), "Invalid object type")
		d.lo:SetText(lo or "")
		d.hi:SetText(hi or "")
	end
	...
	function OptionsSlider:SetTipValueFormat(fmt)
		local d = assert(getWidgetData(self, OptionsSliderData), "Invalid object type")
		assert(fmt == nil or type(fmt) == "string" or type(fmt) == "function",
