...
local edgeSlices = {{"TOPLEFT", 0, -1, "BOTTOMRIGHT", "BOTTOMLEFT", 1, 1}, -- L
...
{"TOPLEFT", 0, 0, "BOTTOMRIGHT", "TOPLEFT", 1, -1}, {"TOPRIGHT", 0, 0, "BOTTOMLEFT", "TOPRIGHT", -1, -1},
...
local function setBackdrop(d, info)
	if info.bgFile then
		...
		local tileH, tileV = not not (info.tileH == nil and info.tile or info.tileH),
...
local function CreateBackdrop(f, info)
	assert(type(f) == "table" and type(f.IsObjectType) == "function" and f:IsObjectType("Frame") and
			   (info == nil or type(info) == "table"), 'Syntax: Create("Backdrop", parent[, props])')
	local r = setmetatable({
		[0] = newproxy()
	}, backdrop_mt)
	setWidgetData(r, BackdropData, {
		self = r,
		host = f,
		edge = XU:Create("ObjectGroup")
	})
	if info then
		r:SetBackdrop(info)
	end
