...
local AB, RW, IM = T.ActionBook:compatible(2, 37), T.ActionBook:compatible("Rewire", 1, 10),
	T.ActionBook:compatible("Imp", 1, 0)
...
local rotationPresentationModes = {
	cycle = 20,
	shuffle = 36,
	random = 52,
	reset = 68,
...
do
	...
	for i, c in ("01234qwertyuiopasdfghjklzxcvbnm5678QWERTYUIOPASDFGHJKLZXCVBNM9"):gmatch("()(.)") do
		sigT[i - 1], sigT[c], sigB[sb(c)], sigN = c, i - 1, i - 1, i
	end
	...
	local function nenc(v, b, rest)
		if b == 0 then
			return v == 0 and rest or error("numeric overflow")
		end
		local v1 = v % sigN
		local v2 = (v - v1) / sigN
	...
	local function venc(v, t, reg)
		...
		elseif type(v) == "number" then
			...
			elseif (v + v == v) or (v < 0) == (v >= 0) then
				...
				t[#t + 1] = sigT[f < 0 and 14 or 13] .. nenc(e + 1500 - 1, 2) ..
								nenc(f * 2 ^ 53 * (f < 0 and -1 or 1), 9)
			end
	...
	local st = {
		[defaultSign] = {"name", "hotkey", "offset", "noOpportunisticCA", "noPersistentCA", "internal", "limit", "id",
						 "skipSpecs", "caption", "icon", "show"}
	}
	...
	function serialize(t, sign)
		...
		local payload = table.concat(tenc(venc(t, {}, setmetatable({}, {
			__index = rt
		}))), "")
	...
	function unserialize(s)
		...
		if st[ssign] == nil or nenc(checksum(ssign .. pri), 7) ~= h then
			return
		end
...
local function RK_SyncRing(name, force, tok)
	...
	if loadLock < 2 or not RK_IsRelevantRingDescription(desc) then
		return
	end
	tok = tok or AB:GetLastObserverUpdateToken("*")
	if not force and tok == desc._lastUpdateToken then
		return
	end
	...
	for i = 1, #desc do
		...
		changed = changed or (action ~= e._action) or action and
					  (e.show ~= e._show or e.embed ~= e._embed or pmode ~= e._pmode)
	...
	changed = changed or (desc._embed ~= desc.embed) or (desc._onOpen ~= onOpenAction) or
				  (desc._onOpenToken ~= onOpenToken)
...
do
	local metaBits = {
		...
		limit = 8,
		embed = 16,
		noOpportunisticCA = 32,
		onOpen = 64
	}
	local ignoreSliceFields = {
		sliceToken = 0,
		vm = 0
	}
	...
	function BUP.PatchSlice(slice, bs, patchBits)
		...
		for s = 1, 2 do
			for k in pairs(s == 1 and bs or slice) do
				...
				elseif tk == "number" and patchAction or tk == "string" and patchMeta and not ignoreSliceFields[k] and
					k:sub(1, 1) ~= "_" then
			...
			if not tokenOK then
				if not isLaxInput then
					...
					assert(false, string.format(
						"desc[%d].sliceToken value is missing, invalid, or not [globally] unique (got %s)", i, tokenDesc),
						4)
				end
...
local function svInitializer(event, _name, sv)
	if event == "LOGOUT" and loadLock > 0 then
		...
		sv.OPieDeletedRings, sv.OPieFlagStore = next(RK_DeletedRings) and RK_DeletedRings,
			next(RK_FlagStore) and RK_FlagStore
	...
	elseif event == "LOGIN" and loadLock == 0 then
		...
		mousemap, SV.OPieDeletedRings, SV.OPieFlagStore = {
			PRIMARY = PC:GetOption("PrimaryButton"),
			SECONDARY = PC:GetOption("SecondaryButton")
		}
		...
		for k, v in pairs(queue) do
			if v.hotkey then
				v.hotkey = v.hotkey:gsub("[^-; ]+", mousemap)
			end
...
function RK_SetRingDesc(name, desc, isLaxInput, doQueueUpgrade)
	...
	elseif desc then
		RK_RingDesc[name], RK_DeletedRings[name] = RK_SanitizeDescription(name, copy(desc), isLaxInput, doQueueUpgrade),
			nil
...
function api:GenFreeRingName(base, reserved)
	assert(type(base) == "string" and (reserved == nil or type(reserved) == "table"),
		'Syntax: name = RK:GenFreeRingName("base"[, reservedNamesTable])', 2)
	base = base:gsub("[^%a%d]", ""):sub(-10)
	if base:match("^OPie") or not base:match("^%a") then
		base = "x" .. base
	end
	local cname, c = base, 1
	while RK_RingDesc[cname] or queue[cname] or SV[cname] or (reserved and reserved[cname] ~= nil) or
		PC:IsKnownRingName(cname) do
...
function api:SetExternalRing(name, desc)
	assert(type(name) == "string" and (type(desc) == "table" or desc == false),
		'Syntax: RK:SetExternalRing("name", descTable or false)', 2)
	assert(queue[name] == nil and (RK_RingDesc[name] == nil or RK_FluxRings[name]),
		"A ring with this name already exists and cannot be modified", 2)
...
function hum:SetMountPreference(groundSpellID, airSpellID, dragonSpellID) -- DEPRECATED [2303/Y8]
	assert(
		(type(groundSpellID) == "number" or not groundSpellID) and (type(airSpellID) == "number" or not airSpellID) and
			(type(dragonSpellID) == "number" or not dragonSpellID),
		'Syntax: groundSpellID, airSpellID = RK:SetMountPreference(groundSpellID|false|nil, airSpellID|false|nil, dragonSpellID|false|nil)',
		2)
...
function private:SetRing(name, desc)
	assert(type(name) == "string" and (type(desc) == "table" or desc == false),
		"Syntax: RK:SetRing(name, descTable or false)", 2)
