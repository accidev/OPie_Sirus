...
do
	...
	function confTextureS(tex, sz, ...) -- (tex, sz, p0, p1, p2)
		...
		if at == "a" then
			pcall(tex.SetAtlas, tex, av, not not asize)
		elseif at == "t" then
			tex:SetTexture(av)
		else
...
function iSB.SetInteractionState(d, state)
	...
	d.Thumb:EnableMouse(enabled and state ~= "THUMB_DRAG")
...
local function CreateScrollBar(name, parent, outerTemplate, id)
	...
	d.Track:EnableMouse(true)
	t:EnableMouse(true)
	d.Thumb = t
	t:SetScript("OnMouseDown", function(_, button)
		iSB.OnTrackMouseDown(d.Track, button)
	end)
	t:SetScript("OnMouseUp", function(_, button)
		iSB.OnTrackMouseUp(d.Track, button)
	end)
